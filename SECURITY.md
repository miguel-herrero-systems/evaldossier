...
Codex transports user-supplied paths and pins as one bounded strict-JSON line over structured non-TTY stdin; if that channel is unavailable, the Skill must stop rather than downgrade to shell interpolation. Claude Code uses one fixed local strict-JSON request file because its Bash surface does not expose the same structured stdin workflow. Its Skill creates that project-relative file with the structured Write tool and invokes one command whose only variable is the host-provided plugin root; no dossier path, pin or source label enters Bash. The runtime opens the final request with no-follow where supported, compares pre-open and opened file identities, reads from the same handle, and rejects duplicate, missing or extra JSON members, malformed or oversized content, unsupported versions, symbolic links and hard links before dossier access. This does not prove that Claude followed the Skill, that a parent directory was never redirected before Write, that the request is confidential, or that a privileged concurrent mutator cannot race the workspace. Concurrent verification invocations in one workspace are unsupported. Use a trusted local working tree, do not place secrets in the request, and treat `.evaldossier-local/` as sensitive local context. A host workspace may not ignore that directory: add it to the workspace's local ignore rules or remove only the exact request file after use.
